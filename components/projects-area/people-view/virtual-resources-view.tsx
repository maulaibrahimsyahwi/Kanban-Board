...
export default function VirtualResourcesView({
  ...
  const resources = [
    { id: 1, name: "rafi", color: "bg-slate-500", type: "per hour" },
  ];

  const handleConvertClick = (res: { id: number; name: string }) => {
    ...
    <div className="h-full flex flex-col">
      <div className="p-6 space-y-6 flex-1 overflow-y-auto">
        <div className="text-sm text-muted-foreground leading-relaxed max-w-4xl">
          Here you manage all virtual resources from your project. They do not
          work on a project. You can only assign tasks to them. A «designer»,
          «developer», «builder» can be a virtual resource. Or for example, a
          «truck», «server», «support». You can create a new resource or choose
          from the list of team’s resources. You can also set costs for them.
        </div>
            ...
            {resources.map((res) => (
              ...
              >
                <div className="col-span-6 flex items-center gap-3">
                  <Checkbox />
                  <div
                    className={`w-8 h-8 rounded-full ${res.color} flex items-center justify-center text-white text-xs uppercase font-medium`}
                  >
